...
export type ConvChain<BaseType, T extends Conv[], From extends ConvType, To extends Exclude<ConvType, From>>
  = T extends [Conv] ? ConvResult<BaseType, Head<T>, From, To> : Head<T> extends Conv ? ConvChain<ConvResult<BaseType, Head<T>, From, To>, Tail<T>, From, To> : never;
...
export function convToObject<Convs extends Conv[], Base>(x: Base, converters: { [P in keyof Convs]: ConvWorker<Convs[P]> }): ConvChain<Base, Convs, 'payload', 'object'> {
  //@ts-ignore
  if(Array.isArray(x)) return x.map(v => { const t = converters.find(c => c.isPayload(v)); return t === undefined ? convToObject<Convs, unknown>(v, converters) : t.payloadToObject(v); }) as ConvChain<Base, Convs, 'payload', 'object'>;
  //@ts-ignore
  if(typeof x === 'object' && x !== null) return Object.fromEntries(Object.entries(x).map(v => { const t = converters.find(c => c.isPayload(v[1])); return [v[0], t === undefined ? convToObject<Convs, unknown>(v[1], converters): t.payloadToObject(v[1])]; })) as ConvChain<Base, Convs, 'payload', 'object'>;
...
export function convToPayload<Convs extends Conv[], Base>(x: Base, converters: { [P in keyof Convs]: ConvWorker<Convs[P]> }): ConvChain<Base, Convs, 'object', 'payload'> {
  //@ts-ignore
  if(Array.isArray(x)) return x.map(v => { const t = converters.find(c => c.isObject(v)); return t === undefined ? convToObject<Convs, unknown>(v, converters) : t.objectToPayload(v); }) as ConvChain<Base, Convs, 'object', 'payload'>;
  //@ts-ignore
  if(typeof x === 'object' && x !== null) return Object.fromEntries(Object.entries(x).map(v => { const t = converters.find(c => c.isObject(v[1])); return [v[0], t === undefined ? convToObject<Convs, unknown>(v[1], converters): t.objectToPayload(v[1])]; })) as ConvChain<Base, Convs, 'object', 'payload'>;
